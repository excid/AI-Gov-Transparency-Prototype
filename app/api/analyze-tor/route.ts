...
export async function POST(request: Request) {
  let form: FormData;
  try { form = await request.formData(); } catch { return Response.json({ error: 'ข้อมูลอัปโหลดไม่ถูกต้อง' }, { status: 400 }); }
  ...
  try {
    ...
    const detail = typeof result.detail === 'string' ? result.detail : 'วิเคราะห์ PDF ไม่สำเร็จ';
    if (!response.ok) return Response.json({ error: detail }, { status: response.status });
    return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
  } catch { return Response.json({ error: 'ระบบวิเคราะห์ TOR ยังไม่พร้อม กรุณาลองใหม่' }, { status: 503 }); }
}
